...
use cryptofolio::db::{
    AccountRepository, RealizedPnLRepository, TaxLotRepository, TransactionRepository,
};
...
async fn test_fifo_buy_sell_workflow() -> Result<()> {
    ...
    let lots = tax_lot_repo
        .list_by_account_asset(account_id, "BTC")
        .await?;
    ...
    assert_eq!(
        pnls.len(),
        2,
        "Should create 2 P&L records (matching 2 lots)"
    );
    ...
    let lots_after = tax_lot_repo
        .list_by_account_asset(account_id, "BTC")
        .await?;
    let remaining_qty: Decimal = lots_after
        .iter()
    ...
    assert_eq!(
        summary,
        dec("25000"),
        "Total realized P&L should be $25,000"
    );
...
async fn test_lifo_vs_fifo_different_results() -> Result<()> {
    ...
    assert_ne!(
        total_lifo,
        dec("25000"),
        "LIFO and FIFO must produce different results"
    );
...
async fn test_unrealized_pnl_calculation() -> Result<()> {
    ...
    let unrealized = pnl_calc
        .calculate_unrealized_pnl(account_id, "BTC", dec("60000"))
        .await?;
    ...
    let unrealized_loss = pnl_calc
        .calculate_unrealized_pnl(account_id, "BTC", dec("35000"))
        .await?;

    assert_eq!(
        unrealized_loss,
        dec("-12500"),
        "Unrealized loss should be -$12,500"
    );
...
async fn test_insufficient_tax_lots_error() -> Result<()> {
    ...
    assert_eq!(
        available,
        dec("1.0"),
        "Original 1.0 BTC should still be available"
    );
...
async fn test_multi_account_pnl_isolation() -> Result<()> {
    ...
    assert_eq!(
        pnl_a,
        dec("20000"),
        "Account A should have $20k realized gain"
    );
    ...
    assert_eq!(
        pnl_b,
        dec("10000"),
        "Account B should have $10k realized gain"
    );
