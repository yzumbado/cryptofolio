...
async fn mock_bitcoin_transactions(world: &mut CryptofolioWorld, incoming: u32, outgoing: u32) {
    ...
    if world.blockchain_mock.is_none() {
        world
            ...
            .await
            .expect("Failed to setup Bitcoin mock");
    }
    ...
    if let Some(mock) = &world.blockchain_mock {
        ...
        mock.mock_address_info(address, 0.5, total_count as u64)
            .await;

        // Mock transactions endpoint
        mock.mock_transactions(address, incoming as i64, outgoing as i64, total_count)
            .await;
    }
}
